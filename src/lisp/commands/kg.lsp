...
(defun gc-kg-label ( / cells par base ang sx sy pts lay stl h prec sep
  ...
  (cond
    ...
    (T
     ...
     ;; ЧИСТИМ ПЕРЕД ПОДПИСЬЮ. Подписи теперь ЗАДАЮТ расчёт: в среднее идут
     ;; ровно подписанные узлы. Значит прежний набор, оставшийся на чертеже,
     ;; молча войдёт в объём - и правило отбора, которое мы только что
     ;; поменяли, не доедет до ведомости. Именно так и вышло на сверке:
     ;; добавленные узлы подействовали, а убранные остались подписанными,
     ;; и квадрат i=2 j=0 не сдвинулся (docs/pitfalls.md -> П75).
     (setq clr (gc-kg-marks-clear))
