...
(defun gc-kg-vol-marked (pts hs / st mp mh lp lh s0 k r z)
  ...
  (if (member nil mh)
    ...
    (progn
      ...
      ;;
      ;; Лечим не заплаткой, а сменой пути: режем фигуру на треугольники
      ;; и делим нулевой линией КАЖДЫЙ. Треугольники не пересекаются
      ;; по построению, поэтому части дают целое всегда.
        ...
        (progn
          (setq *gc-kg-split-bad* (1+ *gc-kg-split-bad*)
                *gc-kg-split-max* (if (> (abs (- (+ (nth 1 r) (nth 3 r)) s0))
                                    ...
                                    (- (+ (nth 1 r) (nth 3 r)) s0)
                                    *gc-kg-split-max*))
          (setq r (gc-kg-vol-parts-m mp mh 1))
          ;; Не сошлось и после треугольников - это уже не вырожденность,
          ;; и молчать об этом нельзя.
          (if (> (abs (- (+ (nth 1 r) (nth 3 r)) s0)) 1.0e-6)
            (setq *gc-kg-split-hard* (1+ *gc-kg-split-hard*)))))
     ...
     (if (> *gc-kg-split-bad* 0)
       (progn
         ...
         (princ (strcat "\n                        (подряд идущие нули на нулевой линии,"
                        ...
                        " м2 - пересчитано"
                        "\n                        через треугольники, части сошлись)"))))
     (if (> *gc-kg-split-hard* 0)
       (progn
         (princ (strcat "\n  [!] НЕ ВЫЛЕЧИЛОСЬ треугольниками: " (itoa *gc-kg-split-hard*)))
         (princ "\n      Части не дают целого даже после разреза на треугольники -")
         (princ "\n      это уже не вырожденность. Объёмы в этих фигурах неверны.")))
     (if (> *gc-kg-tri-lost* 0)
       (progn
