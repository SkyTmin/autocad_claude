...
(setq *gc-kg-dev-min* 0.24)

;; ВИЛКА ПОСЛЕ ЧЕТЫРЁХ ЧЕРТЕЖЕЙ. Наши «лишние» узлы - те, что подписали
;; мы, а образец нет, - легли кучно: 0,224 0,226 0,230 0,232 0,238.
;; Верхняя граница снята с чертежа 1: вершина с отклонением 0,246
;; подписана обоими. Значит порог в (0,238 ; 0,246], поставлено 0,24 -
;; это убирает пять наших лишних из девяти и чертежа 1 не задевает.
;;
...
(defun gc-kg-vol-parts-m (pts hs m / fill sf cut sc tri th p r)
  ...
  (if (and (= m 0) (> (gc-kg-sgn-changes hs) 2))
    (setq m 2))
  (if (and (= m 0)
           (= 1 (gc-kg-get "tri"))
