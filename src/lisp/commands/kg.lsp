...
;;
;; ПОЧЕМУ ОТКЛОНЕНИЕ, А НЕ УГОЛ. Правило снято с образца сверкой наборов
;; узлов (KGC): у него 77 узлов, у нас было 79, и все его нашлись у нас -
;; лишними оказались ровно две вершины. Дальше по девяти вершинам
;; с известным ответом видно, чем он их отличает:
;;
...
;;
;;   вилка по излому     : (16,3 ; 18,0] - КРУГЛОГО ЧИСЛА В НЕЁ НЕ ВЛЕЗАЕТ
;;   вилка по отклонению : (0,059 ; 0,258] - 0,10 м ложится ровно
;;
;; Угол не годится ещё и по смыслу: он не знает длины звеньев. Излом 16°
;; на звене в сантиметр - шум оцифровки, тот же излом на пятиметровом
;; звене - настоящий перелом края. Отклонение это различает само
;; (docs/pitfalls.md -> П77).
(setq *gc-kg-dev-min* 0.10)
                  ...
                  (progn
                    ...
                    (if (> (length pts) 2)
                      (progn
                        ...
                        (setq c (gc-kg-cell-near (gc-kg-centroid pts)))
