...
;;; Команда SV: по 11 точкам тахеометра — два круга (низ/верх сечения)
;;; и стрелки отклонений между центрами.
;;;
;;; Спецификация: specs/001-pile-deviation.md
;;; Загрузка: (load "путь/к/sv.lsp") или через APPLOAD.
...
(setq *gc-pile-z-gap*        0.100)   ; мин разрыв между группами по Z, м
(setq *gc-pile-arrow-len*    0.200)   ; длина Leader, м (см. SPEC-001 §4)
(setq *gc-pile-tol-mm-per-m* 20.0)    ; норматив 20 мм/1м
(setq *gc-pile-text-h*       0.300)   ; высота текста, м
...
;; Делит точки на группы: соседи по Z в одной группе, разрыв > z-gap →
;; новая группа. Возвращает список групп, отсортированных по возрастанию
;; среднего Z (т.е. первая группа — нижнее сечение).
(defun gc-pile-cluster-z (points / sorted groups current prev)
  ...
  (foreach pt sorted
    (cond
      ...
      ((> (- (caddr pt) (caddr prev)) *gc-pile-z-gap*)
...
;; Стиль не создаём — используем что есть.
(defun gc-pile-text-style ( / s)
  (cond
...
;; Стрелка как LINE + SOLID-наконечник. Простая, но управляемая через entmake.
...
(defun gc-pile-draw-arrow-head (start end layer / dx dy len ux uy nx ny size t1 t2)
  ...
  (if (> len 1.0e-9)
    (progn
      ...
      (setq nx (- uy)     ny ux)
      (setq size (* 0.30 len))   ; голова — 30% длины стрелки
      ;; задние углы треугольника: end - size*u ± (size/3)*n
...
(defun gc-pile-draw-text (pt text style layer)
  ;; TEXT с выравниванием Middle Center относительно pt (DXF 11)
  (entmake (list '(0 . "TEXT")
                 (cons 8 layer)
                 (cons 7 style)
                 (cons 10 pt)
                 ...
                 (cons 1 text)
                 (cons 50 0.0)
                 (cons 72 1)        ; гориз. выравнивание: center
                 (cons 11 pt)
                 (cons 73 2))))     ; верт. выравнивание: middle

;;; ====================================================================
;;; ФОРМАТИРОВАНИЕ ПОДПИСЕЙ
;;; ====================================================================

;; Формат подписи под стрелкой — модули (направление показывает стрелка):
;; "12 мм (12 мм/1м)"
(defun gc-pile-mm-pair (raw-mm per-m-mm)
  (strcat (rtos (abs raw-mm)   2 0) " мм ("
          (rtos (abs per-m-mm) 2 0) " мм/1м)"))

;; Со знаком — для сводки в консоли.
...
;; Возвращает список (low-pts high-pts) или nil при ошибке.
    ...
    (T
     ...
     (cond
       ...
       (T
        ...
        (cond
          ((= (length groups) 1)
           (princ "\n[ОШИБКА] Найдено только одно сечение. Используйте режим 2.")
           nil)
          ((= (length groups) 2)
           (princ "\n[!] Точка верха сваи не выделена отдельно. Использую обе группы как сечения.")
           (list (car groups) (cadr groups)))
          ((= (length groups) 3)
           (princ (strcat "\n[i] Точек: " (itoa n)
           ...
           (list (car groups) (cadr groups)))
          (T
           (princ (strcat "\n[ОШИБКА] Обнаружено групп по Z: "
                          (itoa (length groups))
                          ". Это не одна свая. Используйте режим 2."))
           nil)))))))
    ...
    (T
     ...
     (cond
       ...
       (T
        ...
        (entsel)   ; результат игнорируется по SPEC-001 §7
...
(defun gc-pile-process (low-pts high-pts /
  ...
  (cond
    ((null c-low)
     (princ "\n[ОШИБКА] Не удалось построить круг по нижним точкам (все тройки коллинеарны?).")
     nil)
    ((null c-high)
    ...
    (T
     ...
     ;; Слои и стиль текста
     (gc-pile-ensure-layer *gc-l-low*    1)   ; красный
     (gc-pile-ensure-layer *gc-l-high*   3)   ; зелёный
     ...
     ;; Подписи у концов стрелок (смещены чуть дальше за наконечник)
     (gc-pile-draw-text
       (list (+ (car p-end-x)
                (* (+ *gc-pile-text-h* 0.05)
                   (if (>= dx-mm 0) 1.0 -1.0)))
             (cadr p-end-x)
             z-low)
       (gc-pile-mm-pair dx-mm dx-pm) tstyle *gc-l-text*)
     (gc-pile-draw-text
       (list (car p-end-y)
             (+ (cadr p-end-y)
                (* (+ *gc-pile-text-h* 0.05)
                   (if (>= dy-mm 0) 1.0 -1.0)))
             z-low)
       (gc-pile-mm-pair dy-mm dy-pm) tstyle *gc-l-text*)
     ;; Сводка в консоль
     ...
     (if (or (> (abs (caddr c-low))  (+ *gc-pile-r-norm* *gc-pile-r-warn*))
